...
static const struct {
  char *c_name;
  unsigned int c_val;
} facilitynames[] = {{"kern", LOG_KERN},     {"user", LOG_USER},
                     {"mail", LOG_MAIL},     {"daemon", LOG_DAEMON},
                     {"auth", LOG_AUTH},     {"syslog", LOG_SYSLOG},
                     {"lpr", LOG_LPR},       {"news", LOG_NEWS},
                     {"uucp", LOG_UUCP},     {"audit", LOG_AUDIT},
                     {"cron", LOG_CRON},     {"local0", LOG_LOCAL0},
...
#define OPTSTRING                                                              \
  "951yZDNLERKzowefnbvhdkqr:m:p:c:l:s:i:t:u:g:a:x:S:C:A:T:H:Q:I:B:F:G:O:M:X:"  \
  "V:U:j:P:J:W:Y:2:4:6:7:8:0:3:"
...
static struct {
  ...
  char *const arg;
    ...
    {'b', OPT_BOGUSPRIV, NULL,
     gettext_noop("Fake reverse lookups for RFC1918 private address ranges."),
     NULL},
    {'B', ARG_DUP, "<ipaddr>",
    ...
    {'E', OPT_EXPAND, NULL,
     ...
     NULL},
    {'f', OPT_FILTER, NULL,
     gettext_noop("Don't forward spurious DNS requests from Windows hosts."),
     NULL},
    {'F', ARG_DUP, "<ipaddr>,...",
    ...
    {'k', OPT_NO_FORK, NULL,
     gettext_noop("Do NOT fork into the background, do NOT run in debug mode."),
     NULL},
    {'K', OPT_AUTHORITATIVE, NULL,
     gettext_noop("Assume we are the only DHCP server on the local network."),
     NULL},
    {'l', ARG_ONE, "<path>",
    ...
    {LOPT_FORCE, ARG_DUP, "<optspec>",
     gettext_noop("DHCP option sent even if the client does not request it."),
     NULL},
    {'p', ARG_ONE, "<integer>",
     gettext_noop(
    ...
    {'r', ARG_DUP, "<path>",
     gettext_noop("Specify path to resolv.conf (defaults to %s)."), RESOLVFILE},
    ...
    {'W', ARG_DUP, "<name>,<target>,...", gettext_noop("Specify a SRV record."),
     NULL},
    {'w', 0, NULL,
     gettext_noop("Display this message. Use --help dhcp or --help dhcp6 for "
                  "known DHCP options."),
     NULL},
    {'x', ARG_ONE, "<path>",
    ...
    {'2', ARG_DUP, "<interface>",
     gettext_noop("Do not provide DHCP on this interface, only provide DNS."),
     NULL},
    {'3', ARG_DUP, "[=tag:<tag>]...",
...
#ifdef LOPT_ADDR_MAP
    {LOPT_ADDR_MAP, ARG_DUP, "/<domain>/<ipaddr>/<range>",
     gettext_noop("Return ipaddr for all hosts in specified domains."), NULL},
#endif
    {LOPT_BRIDGE, ARG_DUP, "<iface>,<alias>..",
     gettext_noop("Treat DHCP requests on aliases as arriving from interface."),
     NULL},
    {LOPT_SHARED_NET, ARG_DUP, "<iface>|<addr>,<addr>",
     gettext_noop("Specify extra networks sharing a broadcast domain for DHCP"),
     NULL},
    {'5', OPT_NO_PING, NULL,
     gettext_noop("Disable ICMP echo address checking in the DHCP server."),
     NULL},
    {'6', ARG_ONE, "<path>",
     gettext_noop(
    ...
    {LOPT_LUASCRIPT, ARG_DUP, "path",
     gettext_noop("Lua script to run on DHCP lease creation and destruction."),
     NULL},
    ...
    {LOPT_SCRIPT_ARP, OPT_SCRIPT_ARP, NULL,
     gettext_noop("Call dhcp-script with changes to local ARP table."), NULL},
    {'7', ARG_DUP, "<path>",
     gettext_noop("Read configuration from all the files in this directory."),
     NULL},
    {'8', ARG_ONE, "<facility>|<file>",
     gettext_noop("Log to this syslog facility or file. (defaults to DAEMON)"),
     NULL},
    ...
    {LOPT_MATCH, ARG_DUP, "set:<tag>,<optspec>",
     gettext_noop("Set tag if client includes matching option in request."),
     NULL},
    ...
    {LOPT_MINPORT, ARG_ONE, "<port>",
     gettext_noop("Specify lowest port available for DNS query transmission."),
     NULL},
    {LOPT_MAXPORT, ARG_ONE, "<port>",
     gettext_noop("Specify highest port available for DNS query transmission."),
     NULL},
    {LOPT_DHCP_FQDN, OPT_DHCP_FQDN, NULL,
     gettext_noop("Use only fully qualified domain names for DHCP clients."),
     NULL},
    ...
    {LOPT_ADD_SBNET, ARG_ONE, "<v4 pref>[,<v6 pref>]",
     gettext_noop("Add specified IP subnet to forwarded DNS queries."), NULL},
    {LOPT_CPE_ID, ARG_ONE, "<text>",
     gettext_noop("Add client identification to forwarded DNS queries."), NULL},
    {LOPT_DNSSEC, OPT_DNSSEC_PROXY, NULL,
     gettext_noop("Proxy DNSSEC validation results from upstream nameservers."),
     NULL},
    ...
    {LOPT_IGNORE_CLID, OPT_IGNORE_CLID, NULL,
     gettext_noop("Ignore client identifier option sent by DHCP clients."),
     NULL},
    ...
    {LOPT_RA, OPT_RA, NULL,
     gettext_noop("Send router-advertisements for interfaces doing DHCPv6"),
     NULL},
    ...
    {LOPT_AUTHSFS, ARG_DUP, "<NS>[,<NS>...]",
     gettext_noop("Secondary authoritative nameservers for forward domains"),
     NULL},
    {LOPT_AUTHPEER, ARG_DUP, "<ipaddr>[,<ipaddr>...]",
     gettext_noop("Peers which are allowed to do zone transfer"), NULL},
    {LOPT_IPSET, ARG_DUP, "/<domain>[/<domain>...]/<ipset or nfset>...",
     gettext_noop("Specify ipsets to which matching domains should be added"),
     NULL},
    {LOPT_SYNTH, ARG_DUP, "<domain>,<range>,[<prefix>]",
     gettext_noop("Specify a domain and address range for synthesised names"),
     NULL},
    ...
    {LOPT_RA_PARAM, ARG_DUP,
     ...
     gettext_noop("Set MTU, priority, resend-interval and router-lifetime"),
     NULL},
    ...
    {LOPT_REPLY_DELAY, ARG_ONE, "<integer>",
     gettext_noop("Delay DHCP replies for at least number of seconds."), NULL},
...
/* We hide metacharacters in quoted strings by mapping them into the ASCII
   control character space. Note that the \0, \t \b \r \033 and \n characters
   are carefully placed in the following sequence so that they map to
   themselves: it is therefore possible to call unhide_metas repeatedly on
   string without breaking things. The transformation gets undone by
   opt_canonicalise, atoi_check and opt_string_alloc, and a couple of other
   places. Note that space is included here so that
...
static char *split(char *s) { return split_chr(s, ','); }
...
static void do_usage(void) {
  ...
  struct {
    char handle;
    int val;
  } tab[] = {{'$', CACHESIZ}, {'*', EDNS_PKTSZ},           {'&', MAXLEASES},
             {'!', FTABSIZ},  {'#', TFTP_MAX_CONNECTIONS}, {'\0', 0}};
...
static struct server *add_rev4(struct in_addr addr, int msize) {
  ...
  p = serv->domain =
      opt_malloc(29); /* strlen("xxx.yyy.zzz.ttt.in-addr.arpa")+1 */
...
static struct dhcp_netid *dhcp_netid_create(const char *net,
...
static int parse_dhcp_opt(char *errstr, char *arg, int flags) {
  ...
  if (flags == DHOPT_MATCH) {
    if ((new->flags & (DHOPT_ENCAPSULATE | DHOPT_VENDOR)) || !new->netid ||
        new->netid->next)
...
void set_option_bool(unsigned int opt) { option_var(opt) |= option_val(opt); }
...
static int one_opt(int option, char *arg, char *errstr, char *gen_err,
    ...
    do {
      ...
      if (arg && (inet_pton(AF_INET, arg, &new->addr.in.sin_addr) > 0)) {
        ...
        new->addr.in.sin_len = sizeof(new->addr.in);
                 ...
                 inet_pton(AF_INET6, arg, &new->addr.in6.sin6_addr) > 0) {
        new->addr.sa.sa_family = AF_INET6;
...
#endif
  {
      ...
      while (rebind || (end = split_chr(arg, '/'))) {
        ...
        if (regex || nregex) {
          ...
          if (regex) {
            ...
            // Compile the pattern using the PCRE2 API.
            serv->regex = pcre2_compile(
                (PCRE2_SPTR)regex,     // The pattern string
                PCRE2_ZERO_TERMINATED, // The length of the pattern, or
                                       // zero-terminated
                0,                     // Compile options
                &errorcode,            // For returning an error code
                &erroffset,            // For returning an error offset
                NULL);                 // Use default compile context

            if (!serv->regex) {
              PCRE2_UCHAR error_buffer[256];
              // Get the text of the error message.
              pcre2_get_error_message(errorcode, error_buffer,
                                      sizeof(error_buffer));
              my_syslog(LOG_INFO,
                        _("Failed to compile regex pattern %s, error: %s"),
                        regex, (char *)error_buffer);
            }

            serv->flags |= SERV_IS_REGEX;
            // Create a match data block for this pattern, replacing pcre_study.
            serv->match_data =
                pcre2_match_data_create_from_pattern(serv->regex, NULL);
          } else if (nregex) {
            int errorcode;
            PCRE2_SIZE erroffset;
            my_syslog(LOG_INFO, _("Compiling negative regex pattern %s"),
                      nregex);
            // Compile the negative pattern using the PCRE2 API.
            serv->nregex =
                pcre2_compile((PCRE2_SPTR)nregex, PCRE2_ZERO_TERMINATED, 0,
            ...
            if (!serv->nregex) {
              ...
              my_syslog(
                  LOG_INFO,
                  _("Failed to compile negative regex pattern %s, error: %s"),
                  nregex, (char *)error_buffer);
            }

            serv->flags |= SERV_IS_REGEX;
            // Create a match data block for this pattern.
            serv->match_data =
                pcre2_match_data_create_from_pattern(serv->nregex, NULL);
          }
#else
          ret_err("Using a regex while server was configured without regex "
                  "support!");
#endif
        }
        arg = end;
    ...
    if (!arg || !*arg) {
      ...
      char *err = parse_server(arg, &newlist->addr, &newlist->source_addr,
  ...
  case LOPT_IPSET: /* --ipset */
    ...
    ret_err(_("recompile with HAVE_IPSET and/or HAVE_NFSET defined to enable "
              "ipset directives"));
    break;
#else
  {
      ...
      while ((end = split_chr(arg, '/'))) {
        ...
        if ((*arg == ':' && *(real_end - 1) == ':') ||
          ...
          *(real_end - 1) = '\0';
          ...
          // Compile the pattern using the PCRE2 API.
          ipsets->regex =
              pcre2_compile((PCRE2_SPTR)regex,     // The pattern string
                            PCRE2_ZERO_TERMINATED, // The length of the pattern
                            0,                     // Compile options
                            &errorcode, // For returning an error code
                            &erroffset, // For returning an error offset
                            NULL);      // Use default compile context
          ...
          // Save a copy of this regex pattern for debugging.
          ipsets->pattern = (PCRE2_SPTR)strdup(regex);
          ...
          if (!ipsets->regex) {
            ...
            pcre2_get_error_message(errorcode, error_buffer,
                                    sizeof(error_buffer));
            my_syslog(LOG_INFO,
                      _("Failed to compile regex pattern %s, error: %s"), regex,
                      (char *)error_buffer);
          }

          // Create a match data block for this pattern, replacing pcre_study.
          ipsets->match_data =
              pcre2_match_data_create_from_pattern(ipsets->regex, NULL);
  ...
  case LOPT_TFTPPORTS: /* --tftp-port-range */
    if (!(comma = split(arg)) || !atoi_check16(arg, &daemon->start_tftp_port) ||
        ...
        /* dhcp-host has strange backwards-compat needs. */
          ...
          if (pref) {
            ...
            if (!atoi_check(pref, &new_addr->prefixlen) ||
                ...
                ((((u64)1 << (128 - new_addr->prefixlen)) - 1) & addrpart) !=
                    0) {
              dhcp_config_free(new);
          ...
          if ((newhw->hwaddr_len = parse_hex(
                   arg, newhw->hwaddr, DHCP_CHADDR_MAX, &newhw->wildcard_mask,
                   &newhw->hwaddr_type)) == -1) {
            free(newhw);
            dhcp_config_free(new);
            ret_err(_("bad hex constant"));
  ...
  case LOPT_PXE_SERV: /* --pxe-service */
  {
    ...
    char *CSA[] = {"x86PC",    "PC98",       "IA64_EFI",
                   "Alpha",    "Arc_x86",    "Intel_Lean_Client",
                   "IA32_EFI", "x86-64_EFI", "Xscale_EFI",
                   "BC_EFI",   "ARM32_EFI",  "ARM64_EFI",
                   NULL};
  ...
  case '4': /* --dhcp-mac */
  {
    if (!(comma = split(arg)))
      ...
      new->netid.net = opt_string_alloc(set_prefix(arg));
      unhide_metas(comma);
      new->hwaddr_len = parse_hex(comma, new->hwaddr, DHCP_CHADDR_MAX,
                                  &new->mask, &new->hwaddr_type);
      if (new->hwaddr_len == -1) {
        free(new->netid.net);
        ret_err_free(gen_err, new);
      } else {
  ...
  case LOPT_SUBSCR:  /* --dhcp-subscrid */
  {
    ...
    new->netid.net = opt_string_alloc(set_prefix(arg));
    /* check for hex string - must digits may include : must not have nothing
       else, only allowed for agent-options. */
  ...
  case LOPT_NAPTR: /* --naptr-record */
  {
    ...
    if (k < 6 || !(name = canonicalise_opt(a[0])) ||
        !atoi_check16(a[1], &order) || !atoi_check16(a[2], &pref) ||
  ...
  case LOPT_RR: /* dns-rr */
  {
    ...
    if (!atoi_check(comma, &class) || !(new->name = canonicalise_opt(arg)) ||
        (data && (len = parse_hex(data, (unsigned char *)data, -1, NULL,
                                  NULL)) == -1U)) {
      free(new->name);
  ...
  case LOPT_TRUST_ANCHOR: /* --trust-anchor */
  {
    ...
    for (cp = keyhex; *cp;)
      if (isspace(*cp))
        ...
        cp++;
    if ((new->digestlen = parse_hex(keyhex, (unsigned char *)new->digest, len,
                                    NULL, NULL)) == -1) {
      free(new->name);
...
static void read_file(char *file, FILE *f, int hard_opt) {
  ...
  oops:
    ...
    if (errmess || !one_opt(option, arg, daemon->namebuff, _("error"), 0,
...
void read_opts(int argc, char **argv, char *compile_opts) {
  ...
  while (1) {
    ...
    if (option == LOPT_TEST)
      ...
      printf(_("Dnsmasq is free software, and you are welcome to redistribute "
               "it\n"));
...
#define TESTLOOP 2
  ...
  if (!option_bool(OPT_NO_RESOLV) && daemon->resolv_files &&
  ...
  /* If there's access-control config, then ignore --local-service, it's
     intended as a system default to keep otherwise unconfigured installations
     safe. */
