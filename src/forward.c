...
int send_from(int fd, int nowild, char *packet, size_t len,
  ...
  if (!nowild) {
    ...
    if (to->sa.sa_family == AF_INET) {
      ...
      p.ipi6_ifindex =
          iface; /* Need iface for IPv6 to handle link-local addrs */
...
static unsigned int search_servers(time_t now, union all_addr **addrpp,
...

{
  ...
  for (serv = daemon->servers; serv; serv = serv->next) {
    // Check if the server entry uses regex and has a negative pattern compiled.
    if ((serv->flags & SERV_IS_REGEX) && serv->nregex) {
      // Execute the match using the pre-compiled pattern and match_data block.
      int rc = pcre2_match(serv->nregex,        // The compiled negative pattern
                           (PCRE2_SPTR)qdomain, // The domain name to check
                           namelen,             // The length of the domain name
                           0, // Start offset in the subject string
                           0, // Match options
                           serv->match_data, // Block for holding match results
                           NULL);            // Use default match context

      // A return code > 0 indicates a successful match.
      if (rc > 0) {
        // Get a pointer to the offsets vector from the match_data block.
        PCRE2_SIZE *ovector = pcre2_get_ovector_pointer(serv->match_data);

        // ovector[0] is the start of the full match.
        // ovector[1] is the end of the full match.
        domainlen = (unsigned int)(ovector[1] - ovector[0]);
    ...
    /* domain matches take priority over NODOTS matches */
      ...
      *type = SERV_FOR_NODOTS;
      ...
      if (serv->flags & SERV_IS_REGEX) {
        ...
        // Check if a positive regex pattern is compiled for this server.
        if (serv->regex) {
          // Execute the match using the pre-compiled pattern and match_data
          // block.
          int rc =
              pcre2_match(serv->regex,         // The compiled pattern
                          (PCRE2_SPTR)qdomain, // The domain name to check
                          namelen,             // The length of the domain name
                          0, // Start offset in the subject string
                          0, // Match options
      ...
      if (serverhit) {
        if ((serv->flags & SERV_NO_REBIND) && norebind)
          *norebind = 1;
          ...
          /* implement priority rules for --address and --server for same
             domain.
          ...
          if (domainlen >= matchlen) {
            ...
            *domain = serv->domain;
            ...
            if (serv->flags & SERV_NO_ADDR)
              ...
              /* literal address = '#' -> return all-zero address for IPv4 and
               * IPv6 */
              if ((serv->flags & SERV_USE_RESOLV) &&
                ...
                *addrpp = &zero;
...
#ifdef HAVE_SL_ADDR_MAP
                {
                  if ((serv->flags & SERV_ADDR_MAP) == SERV_ADDR_MAP) {
                    uint32_t result =
                        set_forward_ip(serv->net, serv->bits, qdomain);
                    serv->addr.in.sin_addr.s_addr = htonl(result);
                  }
                  *addrpp = (union all_addr *)&serv->addr.in.sin_addr;
                }
...
static int forward_query(int udpfd, union mysockaddr *udpaddr,
  ...
  if (forward || (hash && (forward = lookup_frec_by_sender(ntohs(header->id),
    ...
    /* If we've already got an answer to this query, but we're awaiting keys for
       validation, there's no point retrying the query, retry the key query
       instead...... */
    if (forward->blocking_query) {
      ...
      if (forward->sentto->addr.sa.sa_family == AF_INET)
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV4, "retry",
                  (union all_addr *)&forward->sentto->addr.in.sin_addr,
                  "dnssec");
      else
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV6, "retry",
                  (union all_addr *)&forward->sentto->addr.in6.sin6_addr,
                  "dnssec");
  ...
  if (!flags && forward) {
    ...
    plen = add_edns0_config(header, plen, ((unsigned char *)header) + PACKETSZ,
    ...
    if (option_bool(OPT_DNSSEC_VALID) && do_dnssec) {
      ...
      /* For debugging, set Checking Disabled, otherwise, have the upstream
         check too, this allows it to select auth servers when one is returning
         bad data. */
    ...
    if (find_pseudoheader(header, plen, &edns0_len, &pheader, NULL, NULL)) {
      ...
      /* If we're sending an EDNS0 with any options, we can't recreate the query
       * from a reply. */
      if (edns0_len > 11)
    ...
    while (1) {
      ...
      if (type == (start->flags & SERV_TYPE) &&
        ...
        if (option_bool(OPT_DNSSEC_VALID) &&
            (forward->flags & FREC_ADDED_PHEADER)) {
          /* Difficult one here. If our client didn't send EDNS0, we will have
             set the UDP packet size to 512. But that won't provide space for
             the RRSIGS in many cases. The RRSIGS will be stripped out before
             the answer goes back, so the packet should shrink again. So, if we
             added a do-bit, bump the udp packet size to the value known to be
             OK for this server. We check returned size after stripping and set
        ...
        if (retry_send(sendto(fd, (char *)header, plen, 0, &start->addr.sa,
                              sa_len(&start->addr))))
          continue;
  ...
  if (udpfd != -1) {
    ...
    send_from(udpfd, option_bool(OPT_NOWILD) || option_bool(OPT_CLEVERBIND),
              (char *)header, plen, udpaddr, dst_addr, dst_iface);
  }
...
static size_t process_reply(struct dns_header *header, time_t now,
  ...
  if (daemon->ipsets && extract_request(header, n, daemon->namebuff, NULL)) {
    ...
    for (ipset_pos = daemon->ipsets; ipset_pos; ipset_pos = ipset_pos->next) {
      ...
      if (ipset_pos->domain_type & IPSET_IS_REGEX) {
        ...
        // block.
        int rc = pcre2_match(
            ipset_pos->regex,             // The compiled pattern
            (PCRE2_SPTR)daemon->namebuff, // The subject string to check
            namelen,                      // The length of the subject string
            0,                            // Start offset in the subject
            0,                            // Match options
            ipset_pos->match_data,        // Block for holding match results
            NULL);                        // Use default match context

        // A return code > 0 indicates a successful match.
        if (rc > 0) {
          if (option_bool(OPT_EXTRALOG)) {
            my_syslog(LOG_INFO, _("IPSET Regex Match %s = %s"),
                      daemon->namebuff, ipset_pos->pattern);
          }
          ...
          if (!ipset_pos->nregex) {
            ...
            // It's a negative match, so we stop.
            sets = 0;
            break;
          }
        }
  ...
  if ((pheader = find_pseudoheader(header, n, &plen, &sizep, &is_sign, NULL))) {
    ...
    if (!is_sign) {
      if (added_pheader) {
        /* client didn't send EDNS0, we added one, strip it off before returning
         * answer. */
  ...
  if (!(header->hb4 & HB4_RA) && rcode == NOERROR && server &&
  ...
  if (daemon->bogus_addr && rcode != NXDOMAIN &&
    ...
    if (extract_addresses(header, n, daemon->namebuff, now, sets, is_sign,
                          check_rebind, no_cache, cache_secure, &doctored)) {
...
void reply_query(int fd, int family, time_t now) {
  ...
  ssize_t n = recvfrom(fd, daemon->packet, daemon->packet_buff_sz, 0,
                       &serveraddr.sa, &addrlen);
  size_t nn;
  ...
  /* Determine the address of the server replying  so that we can mark that as
   * good */
...
#ifdef HAVE_DUMPFILE
      dump_packet(DUMP_SEC_QUERY, (void *)header, (size_t)plen, NULL,
                  &start->addr);
#endif

      while (retry_send(sendto(fd, (char *)header, plen, 0, &start->addr.sa,
                               sa_len(&start->addr))))
        ;

      if (start->addr.sa.sa_family == AF_INET)
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV4, "retry",
                  (union all_addr *)&start->addr.in.sin_addr, "dnssec");
      else
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV6, "retry",
                  (union all_addr *)&start->addr.in6.sin6_addr, "dnssec");
  ...
  if ((forward->sentto->flags & SERV_TYPE) == 0) {
    if (RCODE(header) == REFUSED)
      ...
      /* find good server by address if possible, otherwise assume the last one
       * we sent to */
      for (last_server = daemon->servers; last_server;
           last_server = last_server->next)
        if (!(last_server->flags & (SERV_LITERAL_ADDRESS | SERV_HAS_DOMAIN |
                                    SERV_FOR_NODOTS | SERV_NO_ADDR)) &&
  ...
  /* We tried resending to this server with a smaller maximum size and got an
     answer. Make that permanent. To avoid reduxing the packet size for a single
     dropped packet, only do this when we get a truncated answer, or one larger
     than the safe size. */
  ...
  if (forward->forwardall == 0 || --forward->forwardall == 1 ||
    ...
    if ((forward->sentto->flags & SERV_DO_DNSSEC) &&
      ...
      /* We've had a reply already, which we're validating. Ignore this
       * duplicate */
      if (forward->blocking_query)
        return;
      ...
      */
      ...
      while (1) {
        /* As soon as anything returns BOGUS, we stop and unwind, to do
           otherwise would invite infinite loops, since the answers to DNSKEY
           and DS queries will not be cached, so they'll be repeated. */
        if (status != STAT_BOGUS && status != STAT_TRUNCATED &&
            status != STAT_ABANDONED) {
          if (forward->flags & FREC_DNSKEY_QUERY)
            status = dnssec_validate_by_ds(now, header, n, daemon->namebuff,
                                           daemon->keyname, forward->class);
          else if (forward->flags & FREC_DS_QUERY)
            status = dnssec_validate_ds(now, header, n, daemon->namebuff,
                                        daemon->keyname, forward->class);
          else
            status = dnssec_validate_reply(
                now, header, n, daemon->namebuff, daemon->keyname,
                &forward->class,
                !option_bool(OPT_DNSSEC_IGN_NS) &&
                    (forward->sentto->flags & SERV_DO_DNSSEC),
                NULL, NULL, NULL);
#ifdef HAVE_DUMPFILE
        ...
        if (status == STAT_NEED_DS || status == STAT_NEED_KEY) {
          ...
          if (--orig->work_counter == 0 ||
            ...
            *new = *forward; /* copy everything, then overwrite */
            ...
            if (search_servers(now, NULL, F_DNSSECOK, daemon->keyname, &type,
                               &domain, NULL) == 0) {
              ...
              while (1) {
                if (type == (start->flags & (SERV_TYPE | SERV_DO_DNSSEC)) &&
                    ((type & SERV_TYPE) != SERV_HAS_DOMAIN ||
                     hostname_isequal(domain, start->domain)) &&
                    !(start->flags & (SERV_LITERAL_ADDRESS | SERV_LOOP))) {
                  new_server = start;
                  if (server == start) {
            ...
            /* validate routines leave name of required record in
             * daemon->keyname */
            ...
            nn = dnssec_generate_query(
                header, ((unsigned char *)header) + server->edns_pktsz,
...
#ifdef HAVE_DUMPFILE
              dump_packet(DUMP_SEC_QUERY, (void *)header, (size_t)nn, NULL,
                          &server->addr);
#endif

              while (
                  retry_send(sendto(fd, (char *)header, nn, 0, &server->addr.sa,
                                    sa_len(&server->addr))))
                ;
              server->queries++;
            }
    ...
    if ((nn = process_reply(header, now, forward->sentto, (size_t)n,
...
#ifdef HAVE_DNSSEC
      /* We added an EDNSO header for the purpose of getting DNSSEC RRs, and set
         the value of the UDP payload size greater than the no-EDNS0-implied 512
         to have space for the RRSIGS. If, having stripped them and the EDNS0
         header, the answer is still bigger than 512, truncate it and mark it
         so. The client then retries with TCP. */
      if (option_bool(OPT_DNSSEC_VALID) &&
      ...
      send_from(
          forward->fd, option_bool(OPT_NOWILD) || option_bool(OPT_CLEVERBIND),
          daemon->packet, nn, &forward->source, &forward->dest, forward->iface);
    }
    free_frec(forward); /* cancel */
  }
}
...
void receive_query(struct listener *listen, time_t now) {
  ...
  if (n < (int)sizeof(struct dns_header) || (msg.msg_flags & MSG_TRUNC) ||
      (header->hb3 & HB3_QR))
    return;
  ...
  /* We can be configured to only accept queries from at-most-one-hop-away
   * addresses. */
  ...
  if (auth_dns) {
    ...
    if (m >= 1) {
      send_from(listen->fd,
                option_bool(OPT_NOWILD) || option_bool(OPT_CLEVERBIND),
...
static int tcp_key_recurse(time_t now, int status, struct dns_header *header,
  ...
  while (1) {
    ...
    /* limit the amount of work we do, to avoid cycling forever on loops in the
     * DNS */
    if (--(*keycount) == 0)
    ...
    m = dnssec_generate_query(
        new_header, ((unsigned char *)new_header) + 65536, keyname, class,
    ...
    *length = htons(m);
    ...
    retry:
      ...
      if (server->tcpfd == -1) {
        ...
        if (!local_bind(server->tcpfd, &server->source_addr, server->interface,
                        0, 1)) {
          close(server->tcpfd);
          server->tcpfd = -1;
          continue; /* No good, next server */
        ...
        if (!data_sent && connect(server->tcpfd, &server->addr.sa,
      ...
      if (server->addr.sa.sa_family == AF_INET)
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV4, keyname,
                  (union all_addr *)&(server->addr.in.sin_addr),
                  querystr("dnssec-query",
                           new_status == STAT_NEED_KEY ? T_DNSKEY : T_DS));
      else
        log_query(F_NOEXTRA | F_DNSSEC | F_IPV6, keyname,
                  (union all_addr *)&(server->addr.in6.sin6_addr),
                  querystr("dnssec-query",
                           new_status == STAT_NEED_KEY ? T_DNSKEY : T_DS));
      ...
      m = (c1 << 8) | c2;
      new_status = tcp_key_recurse(now, new_status, new_header, m, class, name,
                                   keyname, server, have_mark, mark, keycount);
      break;
    }
...
unsigned char *tcp_request(int confd, time_t now, union mysockaddr *local_addr,
  ...
  while (1) {
    if (query_count == TCP_MAX_QUERIES || !packet ||
    ...
    if ((gotname = extract_request(header, (unsigned int)size, daemon->namebuff,
                                   &qtype))) {
#ifdef HAVE_AUTH
      struct auth_zone *zone;
#endif
...
#endif
    {
      ...
      if (m == 0) {
        ...
        size = add_edns0_config(header, size, ((unsigned char *)header) + 65536,
        ...
        if (!flags && last_server) {
          ...
          /* Loop round available servers until we succeed in connecting to one.
             Note that this code subtly ensures that consecutive queries on this
             connection which can go to the same server, do so. */
          while (1) {
            int data_sent = 0;
          ...
          retry:
            *length = htons(size);
            ...
            if (last_server->tcpfd == -1) {
              if ((last_server->tcpfd = socket(last_server->addr.sa.sa_family,
                                               SOCK_STREAM, 0)) == -1)
                continue;
              ...
              while (retry_send(sendto(
                  last_server->tcpfd, packet, size + sizeof(u16), MSG_FASTOPEN,
                  &last_server->addr.sa, sa_len(&last_server->addr))))
                ;
            ...
            if (!(gotname = extract_request(header, (unsigned int)size,
            ...
            if (last_server->addr.sa.sa_family == AF_INET)
              ...
              log_query(F_SERVER | F_IPV6 | F_FORWARD, daemon->namebuff,
                        (union all_addr *)&last_server->addr.in6.sin6_addr,
                        NULL);
            ...
            if (option_bool(OPT_DNSSEC_VALID) && !checking_disabled &&
              ...
              int keycount =
                  DNSSEC_WORK; /* Limit to number of DNSSEC questions, to catch
                                  loops and avoid filling cache. */
              int status = tcp_key_recurse(
                  now, STAT_OK, header, m, 0, daemon->namebuff, daemon->keyname,
            ...
            /* There's no point in updating the cache, since this process will
               exit and lose the information after a few queries. We make this
               call for the alias and bogus-nxdomain side-effects. */
            /* If the crc of the question section doesn't match the crc we sent,
               then someone might be attempting to insert bogus values into the
               cache by sending replies containing questions and bogus answers.
             */
#ifdef HAVE_DNSSEC
            ...
            m = process_reply(header, now, last_server, (unsigned int)m,
                              ...
                              ad_reqd, do_bit, added_pheader, check_subnet,
                              &peer_addr);
