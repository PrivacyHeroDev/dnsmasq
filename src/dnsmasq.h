...
struct frec {
  ...
  size_t stash_len;
  struct frec
      *dependent; /* Query awaiting internally-generated DNSKEY or DS query */
...
struct dhcp_relay {
  ...
  char *interface; /* Allowable interface for replies from server, and dest for
                      IPv6 multicast */
...
extern struct daemon {
  ...
  unsigned long
      *rr_status; /* ceiling in TTL from DNSSEC or zero for insecure */
  int rr_status_sz;
...
struct crec *cache_insert(char *name, union all_addr *addr,
                          unsigned short class, time_t now, unsigned long ttl,
                          unsigned int flags);
...
int verify(struct blockdata *key_data, unsigned int key_len, unsigned char *sig,
...
void add_update_server(int flags, union mysockaddr *addr,
                       union mysockaddr *source_addr, const char *interface,
                       const char *domain);
...
int address_allocate(struct dhcp_context *context, struct in_addr *addrp,
...
struct dhcp_lease *lease6_find_by_client(struct dhcp_lease *first,
                                         ...
                                         int clid_len, unsigned int iaid);
...
struct dhcp_context *address6_allocate(struct dhcp_context *context,
                                       ...
                                       int temp_addr, unsigned int iaid,
...
struct dhcp_config *find_config(struct dhcp_config *configs,
                                ...
                                char *hostname, struct dhcp_netid *filter);
